...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val result = mapA.plus<String, String>(map = mapB) as MutableMap<String, String>
    for ((key, value) in mapA) {
        if (mapA[key] != result[key]) result[key] = "$value, ${result[key]}"
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    stockPrices.forEach { prices.getOrPut(it.first) { mutableListOf() }.add(it.second) }
 ...
 */
fun canBuildFrom(chars: List<Char>, word: String): Boolean = word.toLowerCase().toSet().all { chars.contains(it) }
...
fun hasAnagrams(words: List<String>): Boolean {
    val map = mutableMapOf<List<Char>, String>()
    for (element in words) {
        if (element.toList().sorted() in map) return true
        else map += element.toList().sorted() to element
    }
    return false
}
